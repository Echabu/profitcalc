document.addEventListener('DOMContentLoaded', () => {
  ...
  const calc = () => {
    ...
    const shippingFee = parseFloat($('shippingFee').value) || 0; // manuel kargo bedeli
    ...
    const revenue = saleInclVAT; // sadece satıştan gelen gelir
    const preCommissionRemaining = revenue - purchaseInclVAT - shippingFee;
    ...
    breakdownLines.push(`<div class="row">Kargo Bedeli: <strong>-${shippingFee.toFixed(2)} TL</strong></div>`);
    ...
    modalLines.push(`<p>${saleInclVAT.toFixed(2)} - ${purchaseInclVAT.toFixed(2)} - ${shippingFee.toFixed(2)} = <strong>${preCommissionRemaining.toFixed(2)} TL</strong></p>`);
    modalLines.push('</li>');
